...
volatile uint16_t timerTriggered = ((uint16_t)0x0000);
...
void EXTI2_3_()
{
	...
	unsigned int signalperiod, signalfreq;
	...
	if ((EXTI->PR & EXTI_PR_PR2) != 0)
	{
		...
		{
			timerTriggered = 1;
			countregister = 0;		  // Reset counter value
		...
		else
		{
			...
			EXTI->IMR &= ~EXTI_IMR_MR1; // Disable interrupt

			countregister = TIM2->CNT; // Read timer counter
			if (countregister > 0)
			{
				signalfreq = (SystemCoreClock / countregister); // Calculate frequency
				signalperiod = (1000000 / signalfreq);			// Calculate period in microseconds
			...
			{
				trace_printf("Timer count is zero, unable to calculate frequency.\n");
			}
